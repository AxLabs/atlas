...
| ----- | ------------------------------------------------------------------- | -------------------------------------------------------------------------------------- | -------------------- | ------------------- | ------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------- | --------- | -------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------ | -------------- | ------------- | ----------- |
| C-001 | Atlas is an open-source frontend platform for Next.js teams         | `README.md`, `docs/public/README.md`, `packages/cli/README.md`                         | Public, contributors | Forkable            | Repository + access model | Public GitHub repository [`blitzcraftlabs/atlas`](https://github.com/blitzcraftlabs/atlas); Apache-2.0 [`LICENSE`](../../LICENSE); `CONTRIBUTING.md` | proven    | Internal `@atlas/*` workspaces remain unpublished npm internals; `@blitzcraftlabs/atlas` is publication-ready and not yet on the registry    | —                                                                        | @thedanielmark | 2026-09-18    | 2026-11-16  |
...
| C-021 | Internal `@atlas/*` workspaces remain private                       | `package.json` in workspaces                                                           | Engineers            | —                   | Package metadata          | Internal workspaces `private: true`; `packages/cli` is `@blitzcraftlabs/atlas` with `publishConfig.access: public`                                   | proven    | Public CLI identity is publication-ready; no npm registry publication has occurred                                                           | —                                                                        | @thedanielmark | 2026-09-16    | 2026-11-16  |
| C-022 | Release automation publishes GitHub Releases after Version PR merge | `.github/workflows/release.yml`, `scripts/publish-atlas-release.mjs`                   | Maintainers          | —                   | CI                        | Fail-closed `publish-atlas-release`; npm job uses OIDC after GitHub Release; no changesets npm `publish:`; no `continue-on-error`                    | qualified | First npm version is canonical `1.0.1` from tag `v1.0.1`; GitHub `v1.0.0` stays immutable; OIDC provenance applies to later publishes        | [releases-and-governance.md](../how-we-build/releases-and-governance.md) | @thedanielmark | 2026-09-18    | 2026-11-16  |
| C-023 | GitHub Release exists for current Atlas version                     | GitHub Releases UI                                                                     | Public               | —                   | Release artifact          | Canonical `v1.0.0` GitHub Release exists; first npm registry version is `1.0.1`                                                                      | proven    | npm package is not published yet; `pnpm dlx @blitzcraftlabs/atlas` is not live; do not bootstrap npm with 0.5.0 or 1.0.0; never retag v1.0.0 | [releases-and-governance.md](../how-we-build/releases-and-governance.md) | @thedanielmark | 2026-09-18    | 2026-11-16  |
