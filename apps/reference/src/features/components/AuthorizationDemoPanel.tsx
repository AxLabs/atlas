...
export function AuthorizationDemoPanel({ session }: AuthorizationDemoPanelProps) {
  ...
  const attemptProtectedDelete = useCallback(async () => {
    setApiResult(null);
    try {
      await apiDelete(`/api/users/${REFERENCE_PERSONA_IDS.user}`);
  ...
  const attemptProtectedUpdate = useCallback(async () => {
    setApiResult(null);
    try {
      await apiPatch(`/api/users/${PROTECTED_REFERENCE_USER_ID}`, { name: "Blocked" });
  ...
  const listUsers = useCallback(async () => {
    setApiResult(null);
    try {
      const response = await apiGet<{ data: { id: string; name: string }[] }>("/api/users");
